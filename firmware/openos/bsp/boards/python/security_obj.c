...
uint8_t security_get_level(OpenMote* self) {
    PyObject* result;
    uint8_t    level;
    
    result  = PyObject_CallObject(self->callback[MOTE_NOTIF_security_get_level],NULL);
    if (result == NULL) {
        printf("[CRITICAL] security_get_level() returned NULL\r\n");
        return;
    }
    
    level = PyInt_AsLong(result);
    
    Py_DECREF(result);
    
#ifdef TRACE_ON
   printf("C@0x%x: security level %d\n",self,level);
#endif
...
uint8_t security_encrypt(OpenMote* self, OpenQueueEntry_t* msg, ieee802154_sec_hdr_t *sec_hdr) {
   PyObject* result;
  
#ifdef TRACE_ON
   printf("C@0x%x: security_encrypt()... \n",self);
#endif

   // forward to Python
   result  = PyObject_CallObject(self->callback[MOTE_NOTIF_security_encrypt],NULL);
   if (result == NULL) {
      printf("[CRITICAL] security_encrypt() returned NULL\r\n");
      return;
   }
   Py_DECREF(result);
