...
#define SENS_ITF_SM_TICK_MS 500000
#define TRACE_ON 1
...
static void sens_itf_core_tick(void)
{
    scheduler_push_task((task_cbt) sens_itf_mote_main, TASKPRIO_SENS_ITF_MAIN);
}

void sens_itf_core_init(void) {
   
    sens_itf_mote_init();
    opentimers_start(SENS_ITF_SM_TICK_MS, TIMER_PERIODIC, TIME_MS, (opentimers_cbt) sens_itf_core_tick);
